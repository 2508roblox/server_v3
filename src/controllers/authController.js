...
const register = asyncHandler(async (req, res) => {
  ...
  return res.status(200).json({
    ...
    let comparePassword = await existUser.isValidPassword(password, existUser.password)
    ...
    const logout = asyncHandler(async (req, res) => {
      res.cookie('refreshToken', '', { expires: new Date(0), httpOnly: true, secure: true }).send('Logged out successfully!');
      });
